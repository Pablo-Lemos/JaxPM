...
class CNN(hk.Module):
    ...
    def __call__(
        ...
        if global_features is not None:
            vmap_features_cic_scale = jax.vmap(
                vmap_features_cic,
                in_axes=(0, 0),
            )
            features_at_pos = vmap_features_cic_scale(features, positions).swapaxes(
                -2, -1
            )
            ...
            broadcast_globals = jnp.broadcast_to(
                ...
                (
                    ...
                    features_at_pos.shape[1],
                    global_features.shape[-1],
                ),
            )
            features_at_pos = jnp.concatenate(
                [features_at_pos, broadcast_globals], axis=-1
            )
        else:
